...
# Set autoreset to True so that styles are automatically reset
init(autoreset=True)
...
class GameBoard:
    ...
    def __init__(self, name: str):
        # 16 characters of input name to ensure the printed nameplate
        self.name = name[:16]

        # Players have both a board and public-facing board
        self.board = []
        self.public_board = []
    ...
    # Called to reset the game board, placing empty spaces
    def clear(self):
        self.board = [[BOARD_WAVE for _ in range(BOARD_SIZE)]
                      for _ in range(BOARD_SIZE)]
        self.public_board = [[BOARD_WAVE for _ in range(BOARD_SIZE)]
                             for _ in range(BOARD_SIZE)]
    ...
    # Check coordinates have already been guessed
...
def main():
    ...
    while True:
        ...
        # Break from the main loop after game
...
def game_loop(player: GameBoard, comp: GameBoard):
    while True:
        ...
        if comp.shoot_coordinates(*prompt_for_coordinates(comp)):
            print(SHOT_STYLE + f"\nYour shot was a hit! {comp.name} \
                  has {comp.ships} ship(s) remaining.")
        ...
        if player.shoot_coordinates(col, row):
            print(SHOT_STYLE + f"\n{comp.name} hit your ship at \
                  {COL_LOOKUP[col]}{row}. You have {player.ships} \
                  ship(s) remaining.")
            ...
            print(SHOT_STYLE + f"\n{comp.name} \
                  shot and missed at {COL_LOOKUP[col]}{row}.")
...
def prompt_for_coordinates(comp: GameBoard):
    while True:
        ...
        if len(user_input) != 2 or not user_input[0].isalpha() \
                or not user_input[1].isnumeric():
...
# Called at the end if the Player plays another round
...
TITLE_STYLE = Fore.BLACK + Back.RED + Style.BRIGHT
LOGO_STYLE = Fore.BLACK + Back.BLUE + Style.BRIGHT
PROMPT_STYLE = Fore.GREEN
ERROR_STYLE = Fore.RED
MESSAGE_STYLE = Back.YELLOW + Fore.BLACK
NAMEPLATE_STYLE = Fore.CYAN + Style.BRIGHT
SHOT_STYLE = Fore.YELLOW
...
LOGO_TEXT = "                             # #  ( )                        \n" \
            ...
            "<---------------'   .  .  .  .  .  .  .  .   '--------------/\n" \
            "  \                                                        / \n" \
            "   \__________________________________________WWS_________/    "
...
INSTRUCTIONS_TXT = "The goal of Battleship is to destroy all enemy ships.\n" \
                   "All ships will be placed randomly on the game board.\n" \
                   "The board consists of columns A-H and rows 0-7.\n" \
                   "Guess enemy ships by typing the coordinates.\n" \
                   "Destroy all 4 enemy ships your the winner!"

GAME_START_TXT = "__________ BEGIN GAME __________"
...
COORDINATE_ERR_MSG = "Invalid Input!\n" \
                     "Please enter only a letter followed by a number.\n" \
                     "Example: C4"

RANGE_ERROR = "Input coordinates are out of range.\n" \
              "Please enter only coordinates A-H and 0-7"
